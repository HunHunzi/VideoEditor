...
export const gVideoFrame = async (
  ...
  // 设置更高的帧率，例如 30fps
  const fps = 30;
  let cmd = `-i ${videoName} -vf fps=${fps} -q:v 5 -s 320x240 -an -preset fast /${frameDir}Frame/%3d.jpeg -hide_banner`;
  let args = cmd.split(" ");
...
/**获取视频的信息 */
export const getVideoInfo = async (fileUrl: string) => {
  ffmpeg.FS("writeFile", "videoInfo", await fetchFile(fileUrl));
  await ffmpeg.run("-i", "videoInfo");
  return videoInfo;
};
